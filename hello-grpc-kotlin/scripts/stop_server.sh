#!/usr/bin/env bash
# Stop script for the kotlin gRPC server. Mirrors the structure of
...
#   proc: "ProtoServerKt"   (the running command we pgrep against)
#
# Usage:
...
FORCE=false
VERBOSE=false
PROCESS_PATTERN="ProtoServerKt"

show_help() {
    cat <<EOF
gRPC Server Stop Script (kotlin)
...
main() {
    ...
    log_info "==============================="
    log_info "gRPC kotlin Server Stop Script"
    log_info "==============================="
    echo
    parse_arguments "$@"
    local signal="TERM"
