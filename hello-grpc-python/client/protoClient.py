...
def execute_unary_call(stub, request):
    ...
    try:
        response = stub.Talk(
            request=request, metadata=DEFAULT_METADATA, timeout=REQUEST_TIMEOUT_SECONDS
        )
...
def execute_server_streaming_call(stub, request):
    ...
    logger.info(
        "Starting server streaming with request: data=%s, meta=%s",
        request.data,
        request.meta,
    )
    ...
    try:
        response_stream = stub.TalkOneAnswerMore(
            request=request, metadata=DEFAULT_METADATA, timeout=REQUEST_TIMEOUT_SECONDS
        )
        ...
        duration = time.time() - start_time
        logger.info(
            "Server streaming completed: received %d responses in %.3fs",
            response_count,
            duration,
        )

    except Exception as e:
        error_mapper.handle_rpc_error(
            e, "TalkOneAnswerMore", {"request_id": request_id}
        )
        raise
...
def execute_client_streaming_call(stub, requests):
    ...
    def request_generator():
        ...
        for request in requests:
            ...
            request_count += 1
            logger.info(
                "Sending client streaming request #%d: data=%s, meta=%s",
                request_count,
                request.data,
                request.meta,
            )
    ...
    try:
        response = stub.TalkMoreAnswerOne(
        ...
        duration = time.time() - start_time
        logger.info(
            "Client streaming completed: sent %d requests in %.3fs",
            request_count,
            duration,
        )
        return response
        ...
        error_mapper.handle_rpc_error(
            e, "TalkMoreAnswerOne", {"request_id": request_id}
        )
        raise
...
def execute_bidirectional_streaming_call(stub, requests):
    ...
    def request_generator():
        ...
        for request in requests:
            ...
            request_count += 1
            logger.info(
                "Sending bidirectional streaming request #%d: data=%s, meta=%s",
                request_count,
                request.data,
                request.meta,
            )
    ...
    try:
        response_stream = stub.TalkBidirectional(
            request_iterator=request_generator(),
            metadata=DEFAULT_METADATA,
            timeout=REQUEST_TIMEOUT_SECONDS,
        )
        ...
        for response in response_stream:
            ...
            response_count += 1
            logger.info(
                "Received bidirectional streaming response #%d:", response_count
            )
            log_response(response)
        ...
        error_mapper.handle_rpc_error(
            e, "TalkBidirectional", {"request_id": request_id}
        )
        raise
...
def log_response(response):
    ...
    for i, result in enumerate(response.results, 1):
        ...
        if not kv:
            logger.info(
                "  Result #%d: id=%d, type=%d, kv=nil", i, result.id, result.type
            )
            continue
        ...
        logger.info(
            "  Result #%d: id=%d, type=%d, meta=%s, id=%s, idx=%s, data=%s",
            i,
            result.id,
            result.type,
            meta,
