...
def _status_from_error(error: Exception):
    """Return a gRPC status code and details while preserving RPC errors."""
    if isinstance(error, grpc.RpcError):
        code = error.code()
        details = error.details() if hasattr(error, "details") else str(error)
        if code is not None:
            return code, details or _get_status_description(code)
    return grpc.StatusCode.INTERNAL, str(error) or "Internal server error"


def set_error_status(context, error: Exception, request_id: str = "") -> None:
    """Set an RPC context status without aborting the current handler."""
    code, details = _status_from_error(error)
    context.set_code(code)
    context.set_details(details)
    handle_rpc_error(error, "server request", {"request_id": request_id})


def abort_with_error(context, error: Exception, request_id: Optional[str] = None):
    """Abort an RPC while preserving an upstream gRPC status when available."""
    code, details = _status_from_error(error)
    handle_rpc_error(error, "server request", {"request_id": request_id or ""})
    return context.abort(code, details)


def retry_with_backoff(
    operation: str, func: Callable[[], T], config: Optional[RetryConfig] = None
) -> T:
    """
    ...
    for attempt in range(config.max_retries + 1):
        ...
        try:
            ...
            if attempt > 0:
                logger.info(
                    f"Operation {operation} succeeded after {attempt + 1} attempts"
                )
            return result
            ...
            if not is_retryable_error(e):
                logger.warning(
                    f"Non-retryable error for {operation}: {map_grpc_error(e)}"
                )
                raise
