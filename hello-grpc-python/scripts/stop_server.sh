#!/usr/bin/env bash
# Stop script for the python gRPC server. Mirrors the structure of
...
#   proc: "protoServer\.py"   (the running command we pgrep against)
#
# Usage:
...
FORCE=false
VERBOSE=false
PROCESS_PATTERN="protoServer\.py"

show_help() {
    cat <<EOF
gRPC Server Stop Script (python)
...
main() {
    ...
    log_info "==============================="
    log_info "gRPC python Server Stop Script"
    log_info "==============================="
    echo
    parse_arguments "$@"
    local signal="TERM"
