#!/usr/bin/env bash
# Stop script for the csharp gRPC server. Mirrors the structure of
...
#   proc: "HelloServer"   (the running command we pgrep against)
#
# Usage:
...
FORCE=false
VERBOSE=false
PROCESS_PATTERN="HelloServer"

show_help() {
    cat <<EOF
gRPC Server Stop Script (csharp)
...
main() {
    ...
    log_info "==============================="
    log_info "gRPC csharp Server Stop Script"
    log_info "==============================="
    echo
    parse_arguments "$@"
    local signal="TERM"
