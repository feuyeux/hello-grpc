...
docker build -f grpc-ts.dockerfile --target server -t feuyeux/grpc_server_ts:1.0.0 .
echo

echo "~~~ build grpc client ts ~~~"
rm -f hello-grpc-ts/hello_server.ts
cp ../hello-grpc-ts/hello_client.ts hello-grpc-ts
docker build -f grpc-ts.dockerfile --target client -t feuyeux/grpc_client_ts:1.0.0 .
rm -rf hello-grpc-ts
echo
