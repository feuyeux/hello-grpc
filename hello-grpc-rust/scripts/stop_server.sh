#!/usr/bin/env bash
# Stop script for the rust gRPC server. Mirrors the structure of
...
#   proc: "(landing_server|target/.*/server)"   (the running command we pgrep against)
#
# Usage:
...
FORCE=false
VERBOSE=false
PROCESS_PATTERN="(landing_server|target/.*/server)"

show_help() {
    cat <<EOF
gRPC Server Stop Script (rust)
...
main() {
    ...
    log_info "==============================="
    log_info "gRPC rust Server Stop Script"
    log_info "==============================="
    echo
    parse_arguments "$@"
    local signal="TERM"
