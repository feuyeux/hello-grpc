...
generate_swift() {
    ...
    # Find Swift protoc plugins
    PROTOC_GEN_SWIFT=$(which protoc-gen-swift || echo "protoc-gen-swift")
    PROTOC_GEN_GRPC_SWIFT=$(which protoc-gen-grpc-swift || echo "protoc-gen-grpc-swift")
...
generate_dart() {
    ...
    cd hello-grpc-app/hello-grpc-flutter || exit

    # Ensure the proto directory exists with symbolic links
    if [ -f ./proto_link.sh ]; then
        ./proto_link.sh
    fi
