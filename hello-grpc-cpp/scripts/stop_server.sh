#!/usr/bin/env bash
# Stop script for the cpp gRPC server. Mirrors the structure of
...
#   proc: "proto_server"   (the running command we pgrep against)
#
# Usage:
...
FORCE=false
VERBOSE=false
PROCESS_PATTERN="proto_server"

show_help() {
    cat <<EOF
gRPC Server Stop Script (cpp)
...
main() {
    ...
    log_info "==============================="
    log_info "gRPC cpp Server Stop Script"
    log_info "==============================="
    echo
    parse_arguments "$@"
    local signal="TERM"
