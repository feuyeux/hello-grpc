#!/usr/bin/env bash
# Stop script for the php gRPC server. Mirrors the structure of
...
#   proc: "hello_server\.php"   (the running command we pgrep against)
#
# Usage:
...
FORCE=false
VERBOSE=false
PROCESS_PATTERN="hello_server\.php"

show_help() {
    cat <<EOF
gRPC Server Stop Script (php)
...
main() {
    ...
    log_info "==============================="
    log_info "gRPC php Server Stop Script"
    log_info "==============================="
    echo
    parse_arguments "$@"
    local signal="TERM"
