#!/usr/bin/env bash
# Stop script for the ts gRPC server. Mirrors the structure of
...
#   proc: "(hello_server|dist/hello_server)"   (the running command we pgrep against)
#
# Usage:
...
FORCE=false
VERBOSE=false
PROCESS_PATTERN="(hello_server|dist/hello_server)"

show_help() {
    cat <<EOF
gRPC Server Stop Script (ts)
...
main() {
    ...
    log_info "==============================="
    log_info "gRPC ts Server Stop Script"
    log_info "==============================="
    echo
    parse_arguments "$@"
    local signal="TERM"
