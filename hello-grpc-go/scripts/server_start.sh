...
# a relative path like ":/Users/han/..." that `go mod tidy` rejects with
...
# need $PWD in GOPATH at all (the GO111MODULE=on line above already
# disables the GOPATH/src workspace mode); we just need a real absolute
# directory.
if [ -z "${GOPATH:-}" ]; then
...
# Preparation steps
log_info "Checking Go gRPC dependencies..."
...
log_info "Starting Go gRPC server..."
