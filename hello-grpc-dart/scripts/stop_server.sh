#!/usr/bin/env bash
# Stop script for the dart gRPC server. Mirrors the structure of
...
#   proc: "(server\.dart|lib/server\.dart)"   (the running command we pgrep against)
#
# Usage:
...
FORCE=false
VERBOSE=false
PROCESS_PATTERN="(server\.dart|lib/server\.dart)"

show_help() {
    cat <<EOF
gRPC Server Stop Script (dart)
...
main() {
    ...
    log_info "==============================="
    log_info "gRPC dart Server Stop Script"
    log_info "==============================="
    echo
    parse_arguments "$@"
    local signal="TERM"
